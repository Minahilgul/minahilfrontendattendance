...
Future<bool> createClass(String name, int? teacherId, String className,
    String students, String subject) async {
...
Future<bool> updateClass(int id, String name, int? teacherId, String className,
    String students, String subject) async {
...
class _ClassesScreenState extends State<ClassesScreen>
  ...
  void _showAddClassDialog() async {
    final result = await showDialog<bool>(
        context: context,
        builder: (context) => AddClassDialog(teachers: _teachersList));
  ...
  void _showEditClassDialog(ClassItem item) async {
    final result = await showDialog<bool>(
        context: context,
        builder: (context) =>
            EditClassDialog(item: item, teachers: _teachersList));
    if (result == true) _loadData();
  }
                      ...
                      builder: (context, _) {
                        ...
                        return ListView.separated(
                          ...
                          itemBuilder: (context, index) => ClassCard(
                            ...
                            onDelete: () =>
                                _showDeleteClassDialog(classes[index]),
                          ),
                        );
                      },
                    ),
...
class ClassCard extends StatelessWidget {
  ...
  IconData _getIconForClass(String name) {
    ...
    if (name.contains('Media') || name.contains('Art'))
        ...
        child: Row(
          ...
          children: [
            Container(
                ...
                child: Icon(_getIconForClass(item.name),
...
class _StatusBadge extends StatelessWidget {
  ...
  const _StatusBadge(
      {required this.label, required this.color, this.isOutlined = false});
...
class _AddClassDialogState extends State<AddClassDialog> {
  ...
  Future<void> _onSave() async {
    if (!_formKey.currentState!.validate() || _selectedTeacherId == null) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: const Text('Please select teacher'),
          backgroundColor: AppColors.danger));
      return;
    }
    setState(() => _isLoading = true);
    ...
    final success = await createClass(
        ...
        "0",
        _subjectController.text.trim()); // NEW: subject passed
    if (!mounted) return;
    setState(() => _isLoading = false);
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: const Text('Failed to create class. Please try again.'),
          backgroundColor: AppColors.danger));
    }
  }
              ...
              DropdownButtonFormField<String>(
                ...
                items: widget.teachers.map((teacher) {
                  ...
                  final String teacherName =
                      teacher['username']?.toString() ?? 'Unknown';
...
class EditClassDialog extends StatefulWidget {
  ...
  const EditClassDialog(
      {super.key, required this.item, required this.teachers});
...
class _EditClassDialogState extends State<EditClassDialog> {
  ...
  void initState() {
    ...
    _subjectController =
        TextEditingController(text: widget.item.subject); // NEW
  ...
  Future<void> _onUpdate() async {
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: const Text('Failed to update class'),
          backgroundColor: AppColors.danger));
    }
  }
